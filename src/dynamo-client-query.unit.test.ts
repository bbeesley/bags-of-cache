...
test.serial('query -> sends the query to dynamo', async (t) => {
...
test.serial('query -> works when queryConcurrency is set', async (t) => {
...
test.serial(
  'query -> does not call dynamo if a fresh copy is cached',
  async (t) => {
    const client = new DynamoCachedClient(parameters);
    const item = await client.query(queryInput);
    t.is(dynamoMock.commandCalls(QueryCommand).length, 1);
...
test.serial(
  'query -> calls dynamo again if the cache has gone stale',
  async (t) => {
...
test.serial(
  'query -> calls dynamo again if the cache params have changed',
  async (t) => {
    const client = new DynamoCachedClient(parameters);
    const item = await client.query(queryInput);
    t.is(dynamoMock.commandCalls(QueryCommand).length, 1);
